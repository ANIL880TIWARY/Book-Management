...
const createReview = async (req, res) => {
    try {
        ...
        await booksModel.findOneAndUpdate({_id:data.bookId,isDeleted:false},{$set:{reviews:book.reviews +1}})
...
const updateReviews = async (req, res) => {
    try {
        ...
        let data = req.body;
        if (Object.keys(data) == 0) { return res.status(400).send({ status: false, message: "Enter data for review" }) }
        if(!validator.isValid(data.review)){ return res.status(400).send({ status: false, message: "Please Insert review" }) }
        if(!validator.isValid(data.reviewedBy)){ return res.status(400).send({ status: false, message: "Please Isert Your Name" }) }
        if(!validator.isValid(data.rating)){ return res.status(400).send({ status: false, message: "Dont forget to Rate" }) }
